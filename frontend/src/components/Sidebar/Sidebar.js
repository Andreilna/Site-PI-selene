...
export default function Sidebar({ isOpen, onClose, isCollapsed }) {
  ...
  const menuItems = {
    principal: [
      ...
      { icon: faLeaf, label: "Monitoramento", href: "/monitoramento" },
      { icon: faBell, label: "Alertas", href: "/alertas" },
    ],
    secundario: [
      { icon: faCamera, label: "Dispositivos", href: "/dispositivos" },
      { icon: faList, label: "Cultivos", href: "/cultivos" },
      { icon: faMicrochip, label: "Admin", href: "/admin" },
    ],
  };
...
<div className={styles.brandSection}>
  ...
  {!isCollapsed && (
    <div className={styles.brandText}>
      <h3 className={styles.brandTitle}>SELENE</h3>
      <p className={styles.brandSubtitle}>Fungicultura Inteligente</p>
    </div>
  )}
</div>
      ...
      {showConfirm && (
        <ConfirmModal
          message="Deseja realmente sair? Você será desconectado da sua conta SELENE."
